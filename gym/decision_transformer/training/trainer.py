...
class Trainer:
    ...
    def __init__(self, model, optimizer, batch_size, get_batch, loss_fn, scheduler=None, eval_fns=None, log_entropy_multiplier = None, multiplier_optimizer = None, multiplier_scheduler = None, entropy_loss_fn=None):
        ...
        self.multiplier_scheduler = multiplier_scheduler
    ...
    def train_iteration(self, num_steps, iter_num=0, print_logs=False):
        ...
        for _ in range(num_steps):
            train_loss, entropy_loss = self.train_step()
            if entropy_loss is not None:
                entropy_losses.append(entropy_loss)
            train_losses.append(train_loss)
            if self.scheduler is not None:
                self.scheduler.step()
            if self.multiplier_scheduler is not None:
        ...
        if self.log_entropy_multiplier is not None:
            logs['training/entropy_multiplier_loss_mean'] = np.mean(entropy_losses)
            logs['training/entropy_multiplier__loss_std'] = np.std(entropy_losses)
